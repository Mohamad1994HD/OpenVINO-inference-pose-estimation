"""
This code is copied from OpenVINO toolkit demo codes
"""
import argparse
import cv2
    ...
    for frame in frames_reader:
        ...
        for pose, bbox in zip(human_poses, bboxes):
            ...
            for id_kpt, kpt in enumerate(pose):
                print("Position: {} id:{}".format((int(kpt[0]),int(kpt[1])), id_kpt))
                cv2.circle(frame, (int(kpt[0]), int(kpt[1])), 6, colors[id_kpt], -1)
